...
def test_Photonics_TM_FDFD_adjoint():
    ...
    abs_problem_sparse = Photonics_TM_FDFD(omega=omega, chi=chi, dl=dl,
    ...
    abs_problem_dense = Photonics_TM_FDFD(omega=omega, chi=chi, dl=dl,
                                        grid_size=(Nx,Ny), pml_size=(Npmlx,Npmly),
                                        chi_background=chi_background,
                                        des_mask=des_mask, ji=ji, sparseQCQP=False)
    ...
    M_E_des = sp.linalg.spsolve(abs_problem_dense.M, 1j*omega*test_src.flatten())[des_mask.flatten()]
    G_E_des = (1j/omega) * abs_problem_dense.G @ test_src[des_mask]
    assert np.allclose(np.linalg.norm(M_E_des-G_E_des), 0, atol=1e-10), "dense setup_EM_operators M and G do not line up."
    ...
    assert np.allclose((obj1-obj0)/delta, grad[ind], rtol=delta*3), "Sparse objective gradient failed finite difference test."
    
    obj0 = abs_problem_dense.structure_objective(dof, grad)
    dof[ind] += delta
    obj1 = abs_problem_dense.structure_objective(dof, [])
    assert np.allclose((obj1-obj0)/delta, grad[ind], rtol=delta*3), "Dense objective gradient failed finite difference test."
